...
def encode_string(data: str, key: str = None) -> str:
    return ''.join([' ' if str == ' ' else key[str] if str in key.keys() else str for str in data])

def decode_string(data: str, key: str = None) -> str:
    return ''.join([''.join([i for i in key if key[i] == str]) for str in data])
...
[Q] Quit program''').lower()
        if inp == 'e':
            ...
            text = input('your string').replace(' ', '').split(',')
            if len(text) > 1:
